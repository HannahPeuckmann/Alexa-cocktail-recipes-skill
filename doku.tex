...
Die Ausgabe des Intents ist der Name eines zufällig ausgewählten Cocktails der Datenbank. Der Intent hat keine Slots, die gefüllt werden müssen. TheCocktailDB bietet die Möglichkeit eines API-Aufrufs für einen zufälligen Cocktail. Nach dem Triggern des RandomCocktailIntents, wird dieser API-Aufruf gemacht und das Session Attribut, das den aktuel aktiven Cocktail speichert, mit dem Namen des zufälligen Cocktails aktualisiert. Im Speech-Output des Intents wird dieser Cocktail genannt, sowie gefragt, ob Interesse daran besteht. Die Antwort auf diese Frage triggert dann einen der von Amazon bereits eingebauten Intents: YesIntent oder NoIntent. In dem Fall, dass der YesIntent nach dem RandomCocktailIntent getriggert wird, delegiert die Lambda-Funktion zum AskForCocktailIntent. Dieser ist dann für den weiteren Dialog zuständig.  \\
...
Der Intent gibt eine kurze Information darüber aus, in welchem Glas ein Cocktail üblicherweise serviert wird. Der Intent hat zwei slots, die beide nicht obligtaorisch sind. Der Intent arbeitet ebenfalls mit Session Atributes, ist der slot 'drink' nicht gefüllt wird auf das Session Attribut für den aktiven Cocktal zurückgrgriffen, falls Benutzer*innen gerade aus einem Anderen Intent kommen, beispielsweise dem AskForCocktailIntent, ist dieses bereits gefüllt. Ist der slot nicht gefüllt und es existert kein Session Attribute wird der slot gefüllt, indem aus der Lambda-Funktion eine Anfrage an Benutzer*innen gestellt wird. Dieser Cocktail ist dann der aktive Cocktail. Hat die Datenbank die gewünschten Informatinen nicht, werden die Benutzer*innen darüber informiert.
...
Der YesMoreInfoIntent wird getriggert durch den build-in AMAZON.YesIntent. Der Intent wird einerseits aktiv, wenn Benutzerinnen im RandomCocktailIntent weitere Informationen über den vorgeschlagenen Cocktail haben möchten. Dann Delegiert der YesMoreInfoIntent an den AskForCocktailIntent weiter, mit dem Cocktail aus dem RandomCocktailIntent als aktiven Cocktail. Darüber hinaus übernimmt der Intent, wenn in einem der beiden filterneden Intents (CocktailWithIngredientIntent und NonAlcoholicCocktailIntent) Benutzer*innen bestätigen, dass sie die gefundenen Cocktails augegeben haben möchten. Dann sucht der Intent drei zufällige Cocktails heraus, vorrausgesetzt es wurden mehr als drei gefunden, andernfalls werden lediglich die gefundenen ausgegeben. Ist nur ein Cocktail gefunden worden, so wird dieser zum aktiven Cocktail. 
...
\speak{ALEXA} I am sorry, I can't handle this request.
\direct{Funktionalität ist nicht eingebaut}
\attrib{2}
...
\direct{An dieser Stelle wird der falsche Intent getriggert, anstelle des CocktailWithIngredientIntent wird der MeasureIntent getriggert. In diesem tritt ebenfalls ein Fehler auf beim füllen der slots auf, "cider as an ingredient" wird als slot-Wert genommen und dementsprechend nicht im Rezept gefunden.}
\attrib{2}
...
\direct{Diese Funktinalität ist nicht eingebaut, die Anfage triggert den AskForCocktailIntent, dieser checkt erst ob der cocktail in der Datenbank vorhanden ist nachdem die obligatorischen slots gefüllt wurden}
\attrib{6}
