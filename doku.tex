...
\speak{ALEXA} You can't do anython wrong with a Snake Bite (UK). Do you agree?
\direct{An dieser Stelle wird der falsche Intent getriggert. Anstatt des NoIntent wird der RepeatIntent getriggert}
\attrib{1}
...
\direct{An dieser Stelle wird der falsche Intent getriggert, anstelle des CocktailWithIngredientIntent wird der MeasureIntent getriggert. In diesem tritt ebenfalls ein Fehler auf, cider wird nicht als Zutat gefunden obwohl es eine Zutat ist.}
\attrib{2}
...
\direct{Hier tritt ein Fehler im AskForCocktailIntent auf, der Cocktail wird nicht in der Datenbank gefunden, obwohl dieser in der Datenbank enthalten ist}
\attrib{5}
...
\attrib{8}
\end{dialogue}
Innsgesammt werden 17 Requests gestellt(abgesehen vom Launch Request), bei 8 Requests treten Probleme auf. Dieser Test war der abschließende, der Skill wurde hinsichtlich der zwei vorangegangenen Tests verbessert. Bei diesem Test passiert es seltener, dass speech to text Fehler auftraten, aufgrund derer der skill scheiterte (3). Häufiger war es der Fall, dass der Falsche Intent getriggert wurde und Probleme mit API Anfragen auftraten (1,2,4,5,6,7,8).
